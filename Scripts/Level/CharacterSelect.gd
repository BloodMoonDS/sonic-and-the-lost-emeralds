...
@export var nextZone = load("res://Scene/Zones/AcornHill.tscn")
var selected = false
...
var levelLabels = ["Acorn Hill Act 1", "Base Zone Act 2"]#, "Chunk Zone Act 1"]
...
func _input(event):
	...
	if !selected:
		...
		if event.is_action_pressed("gm_pause"):
			...
			match(levelID):
				0: # Base Zone Act 1
					Global.nextZone = load("res://Scene/Zones/AcornHill.tscn") # unnecessary since it's arleady set
				#1: # Base Zone Act 2
